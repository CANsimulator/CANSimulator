    ...
    return (
        <div className="container mx-auto max-w-[1600px] space-y-4 px-3 py-4 sm:px-4 sm:py-6">
            <div className="flex flex-wrap items-end justify-between gap-4">
                <div>
                    <h1 className="mb-1 font-mono text-2xl font-black uppercase leading-none tracking-tight text-[#f1f1f1] sm:text-3xl">
                        TEST BENCH
                    </h1>
                    <p className="text-[11px] font-mono font-bold uppercase tracking-[0.2em] text-[#f1f1f1]">
                        CAN Physical Layer Analysis · ISO 11898-2 · Signal Integrity &amp; Power
                    </p>
                </div>
                <div className="hidden items-center gap-3 rounded-lg border border-[#2a2a30] bg-[#1a1a1e] px-3 py-1.5 sm:flex">
                    <div className={`h-1.5 w-1.5 rounded-full ${statusAccent}`} />
                    <span className="text-[10px] font-mono uppercase tracking-wider text-[#f1f1f1]">
                        {benchStatus.label}
                    </span>
                </div>
            </div>
...
const RackLabel: React.FC<{ number: number; name: string; badge?: string }> = ({ number, name, badge }) => (
    <div className="mb-2 flex flex-wrap items-center gap-2 sm:gap-3">
        <span className="rounded border border-[#222] bg-[#111] px-2 py-0.5 text-[11px] font-mono font-black text-[#f1f1f1]">
            {number}U
        </span>
        <span className="text-[12px] font-mono font-black uppercase tracking-widest text-[#f1f1f1]">{name}</span>
        {badge && (
            <span className="rounded border border-[#1a1a20] bg-[#111] px-1.5 py-0.5 text-[10px] font-mono text-[#f1f1f1]">
                {badge}
            </span>
        )}
    </div>
