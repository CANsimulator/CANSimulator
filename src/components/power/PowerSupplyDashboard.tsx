...
export const PowerSupplyDashboard: React.FC = () => {
    ...
    const [history, setHistory] = useState<number[]>(new Array(100).fill(voltage));
    ...
    useEffect(() => {
        ...
        // Clear with dark instrument background
        ...
        const labels = systemVoltage === 12 ? [0, 5, 12] : [0, 12, 24];
        labels.forEach(v => {
            const y = height - (v * scaleY);
            if (y > 10 && y < height - 5) {
                ctx.fillText(`${v}V`, 5, y - 2);
            }
        });
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between mb-6 border-b border-white/5 pb-4 gap-4">
                <div className="flex items-center gap-3">
                    <div 
                        ...
                        role="img"
                        aria-label={`Power State: ${powerState !== 'OFF' ? 'ON' : 'OFF'}`}
                    />
                    <h2 className="text-sm font-black text-dark-950 dark:text-[#f1f1f1] font-sans tracking-widest uppercase transition-colors">
                        Lab Power Supply{' '}
                ...
                <div className="flex bg-black/5 dark:bg-white/5 p-1 rounded-lg border border-black/5 dark:border-white/5 transition-colors">
                    {[12, 24].map(v => (
                        <button
                            ...
                            className={cn(
                                "px-4 py-1 min-h-[44px] rounded text-[10px] font-bold transition-all uppercase tracking-widest font-mono focus:outline-none focus-visible:ring-2 focus-visible:ring-cyber-blue",
                                systemVoltage === v
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 bg-white/40 dark:bg-black/40 rounded-xl p-5 border border-black/5 dark:border-white/5 flex flex-col justify-between relative overflow-hidden shadow-inner transition-colors">
                    ...
                    <div className="absolute top-4 right-4 flex gap-2">
                        <span 
                            className={cn(
                                "text-[9px] px-2 py-0.5 rounded-full font-black font-mono border transition-all",
                                isCC ? "bg-red-500/20 text-red-500 border-red-500/30" : "text-gray-700 border-transparent"
                            )}
                            role="status"
                            aria-label={`Constant Current Mode: ${isCC ? 'Active' : 'Inactive'}`}
                        >CC</span>
                        <span 
                            className={cn(
                                "text-[9px] px-2 py-0.5 rounded-full font-black font-mono border transition-all",
                            ...
                            role="status"
                            aria-label={`Constant Voltage Mode: ${!isCC && powerState !== 'OFF' ? 'Active' : 'Inactive'}`}
                        >CV</span>
                        <span 
                            className={cn(
                                "text-[9px] px-2 py-0.5 rounded-full font-black font-mono border transition-all",
                            ...
                            role="status"
                            aria-label={`Over Voltage Protection: ${isOVP ? 'TRIGGERED' : 'Normal'}`}
                        >OVP</span>
                        <span className={cn(
                        ...
                        <div className={cn(
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <button
                                    ...
                                    className={cn(
                                        "w-12 h-7 rounded-full relative transition-all duration-300 flex items-center min-h-[44px] focus:outline-none focus-visible:ring-2 focus-visible:ring-cyber-blue focus-visible:ring-offset-2 dark:focus-visible:ring-offset-dark-900",
                ...
                <div className="lg:col-span-1 flex flex-col gap-6">
                    ...
                    <div className="space-y-3">
                        ...
                        <button
                            ...
                            className="w-full py-4 min-h-[44px] rounded-lg border border-white/10 bg-white/5 text-gray-300 text-[10px] font-black uppercase tracking-widest font-mono hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed transition-all active:scale-[0.98] focus:outline-none focus-visible:ring-2 focus-visible:ring-cyber-blue"
                        >
