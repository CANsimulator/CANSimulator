...
type CursorMode = 'off' | 'time' | 'voltage';
...
const C = {
    ...
    cursor:    '#ff6b35',
    cursorB:   '#35b0ff',
    isoPass:   'rgba(0,255,136,0.08)',
    isoFail:   'rgba(255,68,68,0.08)',
    isoLine:   'rgba(255,255,255,0.12)',
    dominant:  '#00ff88',
    recessive: '#ffd000',
    eyeGood:   'rgba(0,212,255,0.06)',
    decode:    '#1a1a2e',
};

// ─── Waveform Generator ─────────────────────────────────────
// Generates realistic CAN-like bit stream with proper transition shaping
const BIT_TIME_SAMPLES = 8; // samples per bit period

function generateBitStream(): boolean[] {
    // Generate a realistic CAN frame pattern:
    // SOF(1) + ID(11) + RTR(1) + IDE(1) + r0(1) + DLC(4) + Data(0-64) + CRC(15) + ACK(2) + EOF(7) + IFS(3)
    const bits: boolean[] = [];
    // SOF — always dominant
    bits.push(true);
    // Arbitration field (11-bit ID) — random
    for (let i = 0; i < 11; i++) bits.push(Math.random() > 0.5);
    // RTR — recessive for data frame
    bits.push(false);
    // IDE + r0
    bits.push(false); bits.push(false);
    // DLC (4 bits) — say 8 bytes
    bits.push(true); bits.push(false); bits.push(false); bits.push(false);
    // Data field (8 bytes = 64 bits)
    for (let i = 0; i < 64; i++) bits.push(Math.random() > 0.5);
    // CRC (15 bits)
    for (let i = 0; i < 15; i++) bits.push(Math.random() > 0.5);
    // CRC delimiter — recessive
    bits.push(false);
    // ACK slot — dominant (acknowledged)
    bits.push(true);
    // ACK delimiter — recessive
    bits.push(false);
    // EOF — 7 recessive
    for (let i = 0; i < 7; i++) bits.push(false);
    // Interframe space — 3 recessive
    for (let i = 0; i < 3; i++) bits.push(false);
    return bits;
}
...
function generateSample(prev: Sample | null): Sample {
    // Advance through the frame bit pattern
    ...
    // Edge shaping: transitions have realistic rise/fall characteristics
    const wasTransition = prev ? prev.isDominant !== isDominant : false;
    const edgePhase = bitPhase / BIT_TIME_SAMPLES; // 0..1 within bit

    // Noise
    const n = () => (Math.random() - 0.5) * 0.06;
    // Ringing on edges (decaying oscillation)
    ...
    let canh: number, canl: number;
    if (isDominant) {
        canh = ISO.CANH_DOM_TYP + n() + ringing + overshoot;
        canl = ISO.CANL_DOM_TYP + n() - ringing - overshoot;
    } else {
        canh = ISO.V_REC + n() * 0.5 + ringing * 0.3;
        canl = ISO.V_REC + n() * 0.5 - ringing * 0.3;
    }

    const sample: Sample = {
        canh, canl, isDominant,
        bitIndex: frameBitIndex,
        t: globalSampleIndex,
    };
    globalSampleIndex++;
    return sample;
...
function stepOpt<T>(opts: readonly T[], cur: T, dir: 1 | -1): T {
    ...
    const n = i + dir;
    return n < 0 || n >= opts.length ? cur : opts[n];
}
...
export const VoltageScope: React.FC = () => {
    ...
    const eyeBufferRef = useRef<Sample[][]>([]); // collected bit transitions for eye diagram
    ...
    const draw = useCallback(() => {
        ...
        // ════════════════════════════════════════════
        drawPanel(ctx, M.left, WAVE_Y, PLOT_W, WAVE_H, 'CANH / CANL — Physical Layer', () => {
            // ISO 11898 threshold bands
            drawISOBands(ctx, WAVE_H, ISO.V_MIN, ISO.V_MAX, vw);

            // Grid
            drawGrid(ctx, PLOT_W, WAVE_H, 10, 8, vw);

            // Voltage axis labels
            drawVAxis(ctx, WAVE_H, ISO.V_MIN, ISO.V_MAX, 'V', 1, vw);

            if (samples.length < 2) return;

            // Draw CANL first (behind)
            if (s.ch2.enabled) {
                drawWaveform(ctx, samples, p => p.canl, ISO.V_MIN, ISO.V_MAX, WAVE_H, C.ch2, C.ch2Dim, vw);
            }
            // Draw CANH on top
            if (s.ch1.enabled) {
                drawWaveform(ctx, samples, p => p.canh, ISO.V_MIN, ISO.V_MAX, WAVE_H, C.ch1, C.ch1Dim, vw);
            }
            ...
            ctx.beginPath();
            ctx.moveTo(PLOT_W, trigY);
            ctx.lineTo(PLOT_W + 6, trigY - 4);
            ctx.lineTo(PLOT_W + 6, trigY + 4);
            ctx.fill();
            ...
            if (s.cursorMode === 'time') {
                drawCursor(ctx, s.cursorA * PLOT_W, WAVE_H, C.cursor, 'A', vw);
                drawCursor(ctx, s.cursorB * PLOT_W, WAVE_H, C.cursorB, 'B', vw);
            }
        });
        ...
        // ════════════════════════════════════════════
        drawPanel(ctx, M.left, DIFF_Y, PLOT_W, DIFF_H, 'VDIFF (CANH − CANL) — Differential', () => {
            // ISO threshold bands for differential
            drawDiffThresholds(ctx, DIFF_H, vw);

            drawGrid(ctx, PLOT_W, DIFF_H, 10, 4, vw);
            drawVAxis(ctx, DIFF_H, ISO.DIFF_MIN, ISO.DIFF_MAX, 'V', 0.5, vw);

            if (samples.length < 2) return;

            drawWaveform(ctx, samples, p => p.canh - p.canl, ISO.DIFF_MIN, ISO.DIFF_MAX, DIFF_H, C.diff, C.diffDim, vw);

            // Dominant/recessive threshold lines
            const domY = vToPanel(ISO.VDIFF_DOM_MIN, ISO.DIFF_MIN, ISO.DIFF_MAX, DIFF_H, vw);
            const recY = vToPanel(ISO.VDIFF_REC_MAX, ISO.DIFF_MIN, ISO.DIFF_MAX, DIFF_H, vw);
            ctx.save();
            ctx.setLineDash([2, 2]);
            ctx.lineWidth = 0.6;
            ctx.strokeStyle = C.dominant;
            ctx.globalAlpha = 0.3;
            ctx.beginPath(); ctx.moveTo(0, domY); ctx.lineTo(PLOT_W, domY); ctx.stroke();
            ...
            ctx.globalAlpha = 0.5;
            ctx.textAlign = 'right';
            ctx.fillText('DOM ≥1.5V', PLOT_W - 4, clamp(domY - 3, 8, DIFF_H - 4));
            ctx.fillStyle = C.recessive;
            ctx.fillText('REC ≤0.5V', PLOT_W - 4, clamp(recY + 9, 8, DIFF_H - 4));
            ctx.restore();
        });
        ...
        // PANEL 3: Eye Diagram
        // ════════════════════════════════════════════
        drawPanel(ctx, M.left, EYE_Y, PLOT_W, EYE_H, 'Eye Diagram — Signal Integrity', () => {
            // Eye diagram background
            ctx.fillStyle = 'rgba(0,20,40,0.3)';
            ctx.fillRect(0, 0, PLOT_W, EYE_H);

            drawGrid(ctx, PLOT_W, EYE_H, 8, 4, { zoomX: 1, zoomY: 1, panX: 0, panY: 0 });
        ...
        // ════════════════════════════════════════════
        drawPanel(ctx, M.left, DECODE_Y, PLOT_W, DECODE_H, '', () => {
            if (samples.length < 2) return;

            // Draw bit values along the time axis
            ...
            bitsShown.forEach((info, _bi) => {
                ...
                ctx.lineWidth = 0.5;
                ctx.beginPath();
                ctx.moveTo(info.x1, 0);
                ctx.lineTo(info.x1, DECODE_H);
                ctx.stroke();
            });
        ...
        if (s.ch1.enabled) {
            ctx.fillStyle = C.ch1;
            ctx.fillText(`CH1 ${s.ch1.vdiv}V/div CANH`, hx, 14);
            hx += 140;
        }
        if (s.ch2.enabled) {
            ctx.fillStyle = C.ch2;
            ctx.fillText(`CH2 ${s.ch2.vdiv}V/div CANL`, hx, 14);
            hx += 140;
        }
        if (s.math) {
            ctx.fillStyle = C.diff;
            ctx.fillText(`DIFF CH1−CH2`, hx, 14);
            hx += 120;
        }

        ctx.textAlign = 'right';
        ctx.fillStyle = C.axisText;
        ctx.fillText(`${s.tdiv}µs/div`, CANVAS_W - M.right - 4, 14);

        ctx.fillStyle = s.runMode === 'run' ? C.dominant : '#ff4444';
        ctx.fillText(
            s.runMode === 'run' ? '● RUN' : s.runMode === 'single' ? '● ARMED' : '■ STOP',
            CANVAS_W - M.right - 70, 14
        );
    ...
    function drawPanel(
        ...
        // Clip and translate for content
        ctx.save();
        ctx.beginPath();
        ctx.rect(x, y, w, h);
        ctx.clip();
    ...
    function drawGrid(
        ctx: CanvasRenderingContext2D,
        w: number, h: number, cols: number, rows: number, vw: ViewState
    ) {
        for (let i = 0; i <= cols; i++) {
            const baseX = (i / cols) * w;
            const x = (baseX - w / 2) * vw.zoomX + w / 2 + vw.panX;
        ...
        for (let i = 0; i <= rows; i++) {
            ...
            const y = (baseY - h / 2) * vw.zoomY + h / 2 + vw.panY;
    ...
    // ─── Helper: voltage axis labels ────────────────────────
    function drawVAxis(
        ctx: CanvasRenderingContext2D,
        h: number, vMin: number, vMax: number, unit: string, step: number, vw: ViewState
    ) {
        ctx.font = '9px monospace';
        ctx.fillStyle = C.axisText;
        ctx.textAlign = 'right';
        for (let v = vMin; v <= vMax; v += step) {
            const y = vToPanel(v, vMin, vMax, h, vw);
            if (y < -5 || y > h + 5) continue;
            ctx.fillText(`${v.toFixed(1)}${unit}`, -6, y + 3);
            // tick mark
    ...
    // ─── Helper: ISO 11898 voltage bands on CANH/CANL panel ─
    function drawISOBands(ctx: CanvasRenderingContext2D, h: number, vMin: number, vMax: number, vw: ViewState) {
        // CANH dominant range band
        const cahDomTop = vToPanel(ISO.CANH_DOM_MAX, vMin, vMax, h, vw);
        const cahDomBot = vToPanel(ISO.CANH_DOM_MIN, vMin, vMax, h, vw);
        ctx.fillStyle = 'rgba(0,212,255,0.04)';
        ctx.fillRect(0, cahDomTop, PLOT_W, cahDomBot - cahDomTop);

        // CANL dominant range band
        const calDomTop = vToPanel(ISO.CANL_DOM_MAX, vMin, vMax, h, vw);
        const calDomBot = vToPanel(ISO.CANL_DOM_MIN, vMin, vMax, h, vw);
        ctx.fillStyle = 'rgba(200,80,255,0.04)';
        ctx.fillRect(0, calDomTop, PLOT_W, calDomBot - calDomTop);

        // Recessive center line
        const recY = vToPanel(ISO.V_REC, vMin, vMax, h, vw);
    ...
    // ─── Helper: differential threshold bands ───────────────
    function drawDiffThresholds(ctx: CanvasRenderingContext2D, h: number, vw: ViewState) {
        // Dominant zone (above 1.5V)
        const domLineY = vToPanel(ISO.VDIFF_DOM_MIN, ISO.DIFF_MIN, ISO.DIFF_MAX, h, vw);
        const topY = vToPanel(ISO.DIFF_MAX, ISO.DIFF_MIN, ISO.DIFF_MAX, h, vw);
        ctx.fillStyle = 'rgba(0,255,136,0.03)';
        ctx.fillRect(0, topY, PLOT_W, domLineY - topY);

        // Recessive zone (below 0.5V)
        const recLineY = vToPanel(ISO.VDIFF_REC_MAX, ISO.DIFF_MIN, ISO.DIFF_MAX, h, vw);
        const botY = vToPanel(ISO.DIFF_MIN, ISO.DIFF_MIN, ISO.DIFF_MAX, h, vw);
        ctx.fillStyle = 'rgba(255,208,0,0.03)';
        ctx.fillRect(0, recLineY, PLOT_W, botY - recLineY);
    }

    // ─── Helper: draw a waveform trace with Z-axis intensity ─
    function drawWaveform(
        ...
        for (let i = 0; i < samples.length; i++) {
            const x = sToX(i, samples.length, vw);
            const y = vToPanel(getValue(samples[i]), vMin, vMax, panelH, vw);
            points.push({ x, y });
        }
        ...
        // Z-axis intensity per segment
        for (let i = 1; i < points.length; i++) {
            const dy = Math.abs(points[i].y - points[i - 1].y);
            const velocity = Math.min(dy / 40, 1);
            // Slow beam = bright, fast beam = dim (real phosphor behavior)
        ...
        // White-hot core for flat sections (slow beam)
    ...
    // ─── Helper: draw time cursor ───────────────────────────
    function drawCursor(ctx: CanvasRenderingContext2D, x: number, h: number, color: string, label: string, vw: ViewState) {
        const tx = (x - PLOT_W / 2) * vw.zoomX + PLOT_W / 2 + vw.panX;
        ctx.strokeStyle = color;
        ctx.setLineDash([4, 2]);
        ctx.lineWidth = 1;
        ctx.globalAlpha = 0.7;
    ...
    const computeMetrics = useCallback(() => {
        ...
        const ch1Min = Math.min(...canh);
        const ch1Max = Math.max(...canh);
        const ch2Min = Math.min(...canl);
        const ch2Max = Math.max(...canl);
        ...
        // Count transitions
        let transitions = 0;
        let riseSum = 0, fallSum = 0, riseCount = 0, fallCount = 0;
        let domSamples = 0;
        ...
        const riseTime = riseCount > 0 ? riseSum / riseCount * 20 : 0; // rough ns estimate
        const fallTime = fallCount > 0 ? fallSum / fallCount * 20 : 0;
        const symmetry = riseTime > 0 && fallTime > 0
            ? Math.min(riseTime, fallTime) / Math.max(riseTime, fallTime) * 100
            : 0;
        ...
        setMetrics({
            ...
            vdiff, riseTime: Math.round(riseTime),
            fallTime: Math.round(fallTime),
            symmetry: Math.round(symmetry),
            busLoad: Math.round(busLoad),
            bitRate: Math.round(transitions * 5), // approximate
            eyeWidth: 85 + Math.round(Math.random() * 10), // simulated
    ...
    const updateEyeBuffer = useCallback(() => {
        ...
        // Collect 3-bit-wide windows centered on transitions
        const windowSize = BIT_TIME_SAMPLES * 3;
        for (let i = windowSize; i < samples.length - 1; i++) {
            if (samples[i].isDominant !== samples[i - 1].isDominant) {
                const window = samples.slice(Math.max(0, i - BIT_TIME_SAMPLES), i + BIT_TIME_SAMPLES * 2);
                eyeBufferRef.current.push(window);
                if (eyeBufferRef.current.length > 80) {
                    eyeBufferRef.current = eyeBufferRef.current.slice(-80);
                }
            }
        }
    }, []);
    ...
    const tick = useCallback((time: number) => {
        ...
        if (s.runMode !== 'stop' && time - lastTick.current > interval) {
            const prev = samplesRef.current.length > 0
                ...
                : null;
            const sample = generateSample(prev);
    ...
    useEffect(() => {
        const i1 = setInterval(computeMetrics, 500);
        const i2 = setInterval(updateEyeBuffer, 800);
        return () => { clearInterval(i1); clearInterval(i2); };
    }, [computeMetrics, updateEyeBuffer]);
    ...
    const handleWheel = useCallback((e: React.WheelEvent) => {
        ...
        setView(prev => {
            if (e.shiftKey) {
                return { ...prev, zoomY: clamp(prev.zoomY * f, 0.25, 20) };
            } else if (e.ctrlKey) {
                return { ...prev, zoomX: clamp(prev.zoomX * f, 0.25, 20) };
            }
            return {
                ...prev,
                zoomX: clamp(prev.zoomX * f, 0.25, 20),
                zoomY: clamp(prev.zoomY * f, 0.25, 20),
            };
        });
    }, []);
    ...
    const handlePointerMove = useCallback((e: React.PointerEvent<HTMLCanvasElement>) => {
        ...
        setView(prev => ({
            ...prev,
            panX: panOrigin.current.vx + (e.clientX - panOrigin.current.x) * sx,
            panY: panOrigin.current.vy + (e.clientY - panOrigin.current.y) * sy,
        }));
    }, []);
    ...
    const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
        switch (e.key) {
            case '+': case '=':
                setView(p => ({ ...p, zoomX: clamp(p.zoomX * 1.2, 0.25, 20), zoomY: clamp(p.zoomY * 1.2, 0.25, 20) }));
                break;
            case '-':
                setView(p => ({ ...p, zoomX: clamp(p.zoomX / 1.2, 0.25, 20), zoomY: clamp(p.zoomY / 1.2, 0.25, 20) }));
                break;
            ...
            case ' ':
                e.preventDefault();
                setScope(p => ({ ...p, runMode: p.runMode === 'run' ? 'stop' : 'run' }));
                break;
            case 'Enter':
                e.preventDefault();
                samplesRef.current = [];
                setScope(p => ({ ...p, runMode: 'single' }));
                break;
            case 'c': case 'C':
                setScope(p => ({
                    ...p,
                    cursorMode: p.cursorMode === 'off' ? 'time' : 'off',
                }));
                break;
        }
    }, []);
    ...
    // ─── Controls ───────────────────────────────────────────
    const updateCh = (ch: 'ch1' | 'ch2', u: Partial<ChannelCfg>) =>
        setScope(p => ({ ...p, [ch]: { ...p[ch], ...u } }));
            ...
            <div className="bg-[#0a0a12] rounded-xl border border-[#14142a] shadow-lg overflow-hidden">
                ...
                <div className="flex items-center justify-between px-4 py-2 bg-[#0c0c16] border-b border-[#14142a]">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-1.5 px-2 py-0.5 rounded border"
                            ...
                            <span className="text-[8px] font-mono font-bold" style={{ color: compliance.color }}>
                ...
                <div className="flex flex-col xl:flex-row">
                    ...
                    <div className="flex-1 relative">
                        ...
                        <div className="absolute bottom-1.5 left-1.5 flex gap-1.5 pointer-events-none">
                            <Hint text="Scroll: Zoom" />
                            <Hint text="Ctrl/Shift+Scroll: Axis" />
                            <Hint text="Right-drag: Pan" />
                            <Hint text="C: Cursors" />
                        </div>
                    </div>

                    {/* ── Right Panel: Measurements & Diagnostics ── */}
                    <div className="xl:w-56 p-2 xl:border-l border-[#14142a] flex xl:flex-col gap-2 flex-wrap bg-[#08080f] overflow-y-auto max-h-[620px]">
                        ...
                        <MetricGroup title="ISO 11898 Compliance" icon="📋">
                            <ComplianceRow label="CANH Levels" pass={metrics.isoCANH}
                                detail={`${metrics.ch1Min.toFixed(2)}–${metrics.ch1Max.toFixed(2)}V`} />
                            <ComplianceRow label="CANL Levels" pass={metrics.isoCANL}
                                detail={`${metrics.ch2Min.toFixed(2)}–${metrics.ch2Max.toFixed(2)}V`} />
                            <ComplianceRow label="Differential" pass={metrics.isoDiff}
                                detail={`Vdiff ${metrics.vdiff.toFixed(2)}V`} />
                        </MetricGroup>
                ...
                <div className="px-3 py-2 border-t border-[#14142a] bg-[#0c0c16] flex flex-col lg:flex-row gap-2 flex-wrap">
                    <CtrlGroup label="Acquire">
                        <ScopeBtn label={scope.runMode === 'run' ? 'Stop' : 'Run'}
                    ...
                    <CtrlGroup label="Trigger">
                        <ScopeBtn label={scope.triggerMode} active color={C.trigger}
                            onClick={() => setScope(p => ({
                                ...p,
                                triggerMode: ({ auto: 'SOF', SOF: 'error', error: 'ID', ID: 'auto' } as const)[p.triggerMode],
                            }))} />
                    ...
                    <CtrlGroup label="Cursors">
                        <ScopeBtn
                            label={scope.cursorMode === 'off' ? 'Off' : 'ΔT/ΔV'}
                            active={scope.cursorMode !== 'off'}
                            color={C.cursor}
                    ...
                    <CtrlGroup label="Persist">
                        <ScopeBtn
                            label={scope.persistence ? 'ON' : 'OFF'}
...
const Stepper: React.FC<{ label: string; value: string; onUp: () => void; onDown: () => void }> = ({ label, value, onUp, onDown }) => (
    <div className="flex items-center">
        <button onClick={onDown}
            className="w-5 h-6 flex items-center justify-center bg-[#0a0a12] border border-[#1a1a2e] rounded-l text-gray-600 hover:text-white hover:bg-[#14142a] transition-all text-[9px] font-mono">◀</button>
        ...
        <button onClick={onUp}
            className="w-5 h-6 flex items-center justify-center bg-[#0a0a12] border border-[#1a1a2e] rounded-r text-gray-600 hover:text-white hover:bg-[#14142a] transition-all text-[9px] font-mono">▶</button>
    </div>
);
...
const MetricRow: React.FC<{
    ...
    <div className="flex justify-between items-baseline">
        ...
        <div className="flex items-center gap-1">
            {status && (
                <span className={`w-1.5 h-1.5 rounded-full ${status === 'pass' ? 'bg-emerald-400' : status === 'warn' ? 'bg-amber-400' : 'bg-red-400'}`} />
            )}
