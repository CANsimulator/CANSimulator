...
export function BitTimingConfig() {
    ...
    const handleHexEdit = (reg: 'BTR0' | 'BTR1', hex: string): { success: boolean; error?: string } => {
        const val = parseInt(hex.replace('0x', ''), 16);
        if (isNaN(val) || val < 0 || val > 255) return { success: false, error: 'Invalid range' };
        ...
        if (nextTiming.sjw > nextSjwMax) {
            ...
            return { success: false, error: 'SJW Constraint Violation' };
        }
        ...
        <div className="bg-white dark:bg-[#1a1a1e] rounded-2xl border border-black/10 dark:border-[#2a2a30] p-4 shadow-sm dark:shadow-[inset_0_2px_4px_rgba(0,0,0,0.5)] transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                ...
                <div className="md:col-span-7 xl:col-span-8 space-y-4">
                    ...
                    {/* ─── Parameter Adjustment (Grid Layout) ─── */}
                    <div className="p-4 rounded-xl bg-white/5 dark:bg-[#0c0c0e] border border-white/10 transition-all">
                        <span className="text-[10px] font-mono font-bold text-white/50 uppercase tracking-widest block mb-4">SEGMENT CONFIGURATION (TQ Allocation)</span>

                        {/* 2x2 Grid for Segment Sliders */}
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                            <div onMouseEnter={() => setHoveredField('TSEG1')} onMouseLeave={() => setHoveredField(null)}>
                                <SegmentSlider
                                    label="PROP SEG"
                                    color="#3b82f6"
                                    min={1}
                                    ...
                                    description="Prop_Seg (TS1): Compensates for physical delay times within the network."
                                />
                            </div>
                            <div onMouseEnter={() => setHoveredField('TSEG1')} onMouseLeave={() => setHoveredField(null)}>
                                <SegmentSlider
                                    ...
                                    description="Phase_Seg1: Buffer before the sample point used for positive resync."
                                />
                            </div>
                            <div onMouseEnter={() => setHoveredField('TSEG2')} onMouseLeave={() => setHoveredField(null)}>
                                <SegmentSlider
                                    label="PHASE SEG 2"
                                    color="#bf00ff"
                                    min={1}
                                    ...
                                    description="Phase_Seg2: Buffer after the sample point used for negative resync."
                                />
                            </div>
                            <div onMouseEnter={() => setHoveredField('SJW')} onMouseLeave={() => setHoveredField(null)}>
                                <motion.div animate={constraintViolation ? 'shake' : 'normal'} variants={shakeVariants}>
                                    <SegmentSlider
                                        label="SJW"
                                    ...
                                    />
                                    {!sjwValid && (
                                        <div role="alert" className="mt-2 p-1.5 rounded bg-red-500/10 border border-red-500/20">
                                            <span className="text-[10px] font-mono text-red-400 flex items-center gap-1">
                                                <AlertTriangle size={10} /> SJW exceeds min(PH1,PH2)
                                            </span>
                                        </div>
                                    )}
                                </motion.div>
                        ...
                        {/* 3-Column Grid for Global Parameters */}
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="space-y-2" onMouseEnter={() => setHoveredField('BRP')} onMouseLeave={() => setHoveredField(null)}>
                                <div className="flex items-center justify-between">
                                    <Tooltip content="Baud Rate Prescaler">
                                        <span className="text-[9px] font-mono font-bold uppercase tracking-wider text-white/40 cursor-help">
                                            PRESCALER (BRP)
                                        </span>
                                    </Tooltip>
                                    <span className="text-[9px] font-mono text-white/30">1 – 64</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <button
                                        onClick={() => handleChange('brp', Math.max(1, timing.brp - 1))}
                                        disabled={timing.brp <= 1}
                                        className="w-10 h-10 flex items-center justify-center rounded bg-white/5 border border-white/10 text-white font-mono text-[12px] font-bold hover:bg-white/10 disabled:opacity-50 transition-colors"
                                    >−</button>
                                    <input
                                        type="text"
                                        value={brpInput}
                                        ...
                                        className={cn(
                                            "flex-1 h-10 text-center bg-white/5 border text-white font-mono text-[11px] font-bold outline-none transition-all",
                                            brpError ? "border-red-500/50" : "border-white/10 focus:border-cyber-blue"
                                        )}
                                    />
                                    <button
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                    <Tooltip content="Oscillator Frequency">
                                        <span className="text-[9px] font-mono font-bold uppercase tracking-wider text-white/40 cursor-help">CLOCK (Fosc)</span>
                                    </Tooltip>
                                </div>
                                    ...
                                    {[8, 16, 20, 24, 40].map(f => (
                                        <button
                                            ...
                                            className={`px-1 py-1.5 rounded text-[10px] font-mono font-bold border transition-all ${timing.oscillator === f * 1_000_000
                                                ? 'bg-cyber-blue/10 text-cyber-blue border-cyber-blue/40'
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                    <Tooltip content="Triple sampling improves noise immunity">
                                        <span className="text-[9px] font-mono font-bold uppercase tracking-wider text-white/40 cursor-help">SAMPLE MODE</span>
                                    </Tooltip>
                                </div>
                                <button
                                ...
                                >
                                    <div className={`w-8 h-4 rounded-full relative transition-colors ${timing.sam === 1 ? 'bg-amber-500' : 'bg-white/10'}`}>
                                        <motion.div
                                            className="absolute top-0.5 w-3 h-3 rounded-full bg-white shadow-sm"
                                            animate={{ left: timing.sam === 1 ? '1.1rem' : '0.125rem' }}
                                        />
                                    </div>
                                    <span className={`text-[10px] font-mono font-bold transition-colors ${timing.sam === 1 ? 'text-amber-500' : 'text-white/40'}`}>
                                        {timing.sam === 1 ? 'TRIPLE' : 'SINGLE'}
                                    </span>
                                </button>
                            </div>
                        </div>
...
function PresetTabStrip({ presets, activePreset, onApply }: {
    ...
    return (
        <div className="p-3 rounded-lg bg-white/5 border border-white/5 mb-4 transition-colors">
            <span className="text-[10px] font-mono font-bold text-white/40 uppercase tracking-widest block mb-2">BAUD RATE PRESETS</span>
            <div className="pb-1">
                <div className="flex flex-wrap gap-2 w-full">
                    {presets.map(preset => (
                        <button
                            ...
                            className="relative flex-none px-3 py-2.5 rounded text-[10px] font-mono font-bold transition-all active:scale-95 min-h-[44px]"
                            style={{
                                color: activePreset === preset.name ? 'var(--cyber-blue)' : 'rgba(255,255,255,0.4)',
                        ...
                        >
                            ...
                            {activePreset === preset.name && (
                                <motion.div
                                    ...
                                    className="absolute inset-0 rounded bg-cyber-blue/5 border border-cyber-blue/30"
...
function SegmentSlider({ label, color, value, min = 1, max, onChange, description }: SegmentSliderProps) {
    ...
    const handlePointerDown = (e: React.PointerEvent) => {
        ...
        const newValue = Math.max(min, Math.min(max, Math.round(percent * (max - min) + min)));
        onChange(newValue);
        setIsDragging(true);
    ...
    return (
        <div className="space-y-2">
            <div className="flex items-center justify-between">
                ...
                <span className="text-[9px] font-mono text-white/30">{min} – {max}</span>
            </div>
            ...
            <div
                ...
                className="relative h-8 bg-white/5 border border-white/5 rounded cursor-pointer group transition-colors"
                tabIndex={0}
                role="slider"
            ...
            >
                ...
                <motion.div
                    ...
                    animate={{ width: `${((value - min) / Math.max(1, max - min)) * 100}%` }}
            ...
            <span className="text-[10px] font-mono text-white/30 block transition-colors leading-tight">{description}</span>
        </div>
    );
}
...
function RegisterRow({ name, hex, binary, fields, onCopy, onEdit, isCopied, isExternalShaking, highlightedField }: {
    ...
    onEdit: (hex: string) => { success: boolean; error?: string };
    isCopied: boolean;
    ...
    const submitEdit = () => {
        if (isValidHex(editVal)) {
            ...
            const result = onEdit(clean.toUpperCase());
            if (result.success) {
                setIsEditing(false);
                setIsInvalid(false);
                setHexError('');
                ...
                setIsShaking(true);
                setHexError(result.error || 'Internal error');
    ...
    return (
        <div className="mb-3 last:mb-0">
            <div className="flex items-center justify-between mb-1.5 px-1">
                <div className="flex items-center gap-2">
                    <span className="text-[10px] font-mono font-black text-white">{name}</span>
                        ...
                        {isEditing ? (
                            <div className="relative">
                                <motion.input
                                    ...
                                    className={cn(
                                        "w-12 bg-white/10 border outline-none px-1 rounded transition-all font-mono font-bold text-[10px]",
                                        hexError 
                                ...
                                />
                                {hexError && (
                                    <span role="alert" className="absolute top-full left-0 text-[8px] font-mono text-red-400 block mt-0.5 whitespace-nowrap z-20">
                                        {hexError}
                                    </span>
                                )}
                            </div>
                            ...
                            <span
                                ...
                                className="cursor-pointer hover:text-cyber-blue transition-colors border-b border-dashed border-white/20"
                            >
                                0x{hex}
                            </span>
                        )}
                ...
                <button
                    ...
                    className={`text-[8px] font-mono font-bold uppercase tracking-widest px-1.5 py-0.5 rounded transition-all ${
                        isCopied 
                            ? 'bg-cyber-blue/20 text-cyber-blue' 
                            : 'text-white/30 hover:text-cyber-blue hover:bg-cyber-blue/5'
                        }`}
                >
                ...
                {binary.split('').map((bit, i) => {
                    ...
                    return (
                        <div
                            ...
                            style={{ 
                                boxShadow: isHighlighted ? `inset 0 0 0 1px ${field?.color ?? '#00f3ff'}80, 0 0 8px rgba(0,243,255,0.3)` : undefined,
                        ...
                        >
                            <span className={cn(
                                ...
                                isHighlighted ? "text-white" : (bit === '1' ? "text-white" : "text-white/20")
                            )}>
                                {bit}
                            </span>
                            {field && (
                            ...
                            <span className={cn(
                                ...
                                isHighlighted ? "opacity-100 text-white font-bold" : "opacity-0 group-hover:opacity-100 text-white/30"
                            )}>
                                {7 - i}
                            </span>
                        </div>
                        ...
                        {f.tooltip ? (
                            <Tooltip content={f.tooltip}>
                                <div className="flex items-center gap-1">
                                    <span className="text-[8px] font-mono font-bold text-white/40 transition-colors uppercase">{f.name}</span>
                                    <span className="text-[8px] font-mono text-white/20 transition-colors">{f.bits}</span>
                                </div>
                            </Tooltip>
                        ) : (
                            <>
