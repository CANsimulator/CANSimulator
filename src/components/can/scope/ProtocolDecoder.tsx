...
export const ProtocolDecoder: React.FC<ProtocolDecoderProps> = ({ frames, onRowClick }) => {
    return (
        <div className="glass-panel flex flex-col h-[280px] overflow-hidden">
            <div className="flex items-center justify-between p-4 bg-white/5 border-b border-white/5">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-[10px] font-mono font-black uppercase tracking-[0.2em] text-[#00f3ff]">CAN Protocol Decoder</h3>
                    <span className="text-[10px] font-mono font-bold text-white/30 uppercase tracking-widest px-2 py-0.5 rounded-full border border-white/5">ISO 11898-1</span>
                </div>
                <div className="flex items-center gap-4 text-[9px] font-mono font-black text-white/40 uppercase tracking-widest">
                    ...
                    <span className="flex items-center gap-1.5 text-[#00ff9f]"><ShieldCheck size={10} /> 100% Valid</span>
                </div>
            </div>

            <div className="flex-1 overflow-y-auto no-scrollbar">
                <table className="w-full text-left border-collapse">
                    <thead className="sticky top-0 bg-[#0a0a0f]/90 backdrop-blur-md z-10 border-b border-white/5">
                            ...
                            {frames.map((frame, i) => (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-2.5">
                                        <span className={cn(
                                            "text-[9px] font-mono font-black border px-1.5 py-0.5 rounded",
                                        ...
                                        <div className="flex items-center gap-2">
                                            {frame.status === 'ok' ? (
                                                <div className="flex items-center gap-1.5 text-[#00ff9f]/80 text-[10px] font-mono font-black uppercase tracking-widest">
                                                    <div className="h-1 w-1 rounded-full bg-[#00ff9f]" />
                                                    ACK
                                                </div>
                                            ) : (
                ...
                {frames.length === 0 && (
                    <div className="flex flex-col items-center justify-center p-20 gap-3 opacity-20">
                        <FileCheck size={40} />
                        <p className="text-sm font-mono font-black uppercase tracking-[0.2em]">Capture Data Pending...</p>
                    </div>
                )}
            </div>
        </div>
